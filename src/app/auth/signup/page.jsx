...
import { Eye, EyeOff, Mail, Lock, User, CheckCircle, XCircle, ArrowLeft, Sparkles } from 'lucide-react';
import { authClient } from '@/lib/auth-client';
import { useTheme } from '@/context/ThemeContext';
import Link from 'next/link';

const PasswordCheckItem = ({ passed, text, isLight }) => (
    <div className="flex items-center space-x-2 text-xs transition-all duration-200">
        {passed ? (
            <CheckCircle className="w-3.5 h-3.5 text-green-500 flex-shrink-0" />
        ) : (
            <XCircle className="w-3.5 h-3.5 text-gray-400 flex-shrink-0" />
        )}
        <span style={{ color: passed ? '#10b981' : isLight ? '#94a3b8' : '#6b7280' }}>{text}</span>
    </div>
);
    ...
    const handleChange = (e) => {
        ...
        if (name === 'password' || name === 'confirmPassword') {
            ...
            setPasswordChecks({
                ...
                passwordsMatch: pass === confirm && confirm.length > 0,
            });
        }
    };
        ...
        try {
            ...
            if (error) {
                ...
                if (error.code === 'USER_ALREADY_EXISTS') {
                    ...
                    setError(error.message || error.statusText || 'Registration failed. Please try again.');
                }
                return;
            }
                ...
                <form className="space-y-4" onSubmit={handleSignUp}>
                    ...
                    <div>
                        ...
                        {formData.password.length > 0 && (
                            <div className="mt-3 space-y-1.5 pl-1 animate-fadeIn">
                                <PasswordCheckItem passed={passwordChecks.minLength} text="At least 8 characters" isLight={isLight} />
                                <PasswordCheckItem passed={passwordChecks.hasUpper} text="One uppercase letter (A-Z)" isLight={isLight} />
                                <PasswordCheckItem passed={passwordChecks.hasLower} text="One lowercase letter (a-z)" isLight={isLight} />
                                <PasswordCheckItem passed={passwordChecks.hasNumber} text="One number (0-9)" isLight={isLight} />
                            </div>
                        )}
                    </div>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                style={{
                                    backgroundColor: isLight ? 'var(--bg-input)' : '#1a1a1a',
                                    borderColor: formData.confirmPassword && passwordChecks.passwordsMatch
                                        ? '#10b981'
                                        : formData.confirmPassword && !passwordChecks.passwordsMatch
                                            ? '#ef4444'
                                            : isLight ? 'var(--border-color)' : 'rgba(31,41,55,0.6)',
                                    color: isLight ? 'var(--text-primary)' : '#f1f5f9',
                                }}
                                placeholder="••••••••"
                                onFocus={(e) => e.target.style.borderColor = '#6366f1'}
                                onBlur={(e) => {
                                    if (!passwordChecks.passwordsMatch && formData.confirmPassword) {
                                        e.target.style.borderColor = '#ef4444';
                                    } else if (passwordChecks.passwordsMatch) {
                                        e.target.style.borderColor = '#10b981';
                                    } else {
                                        e.target.style.borderColor = isLight ? 'var(--border-color)' : 'rgba(31,41,55,0.6)';
                                    }
                                }}
                            />
                            <button
                        ...
                        {passwordChecks.passwordsMatch && (
                            <p className="text-xs mt-1.5 pl-1 flex items-center space-x-1" style={{ color: '#10b981' }}>
